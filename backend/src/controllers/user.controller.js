...
export async function getMyFriends(req, res) {
  try {
    const user = await User.findById(req.user.id)
      .select("friends")
      .populate("friends", "fullName profilePic skillsOffered skillsWanted");
...
export async function getFriendRequests(req, res) {
  try {
    const incomingReqs = await FriendRequest.find({
      ...
      status: "pending",
    }).populate("sender", "fullName profilePic skillsOffered skillsWanted");
...
export async function getOutgoingFriendReqs(req, res) {
  try {
    const outgoingRequests = await FriendRequest.find({
      sender: req.user.id,
      status: "pending",
    }).populate("recipient", "fullName profilePic skillsOffered skillsWanted");
...
export async function rejectFriendRequest(req, res) {
  try {
    ...
    if (friendRequest.recipient.toString() !== req.user.id) {
      console.log("Unauthorized: Current user is not recipient");
    ...
    console.error("Error in rejectFriendRequest controller:", error); // log full error object
    res.status(500).json({ message: "Internal Server Error" });
  }
}
