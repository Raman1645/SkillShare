...
export async function onboard(req, res) {
  try {
    ...
    const { fullName, bio, skillsOffered, skillsWanted } = req.body;

    if (!fullName || !bio || !skillsOffered || !skillsWanted) {
    ...
    // Optional: Validate that skillsOffered and skillsWanted are non-empty arrays
    if (!Array.isArray(skillsOffered) || skillsOffered.length === 0 ||
      !Array.isArray(skillsWanted) || skillsWanted.length === 0) {
      return res.status(400).json({
        message: "skillsOffered and skillsWanted must be non-empty arrays",
      });
    }
